...
extern double escape_time2;
extern double degree_135_time;
//--------------------------------------------------//
                ...
                if(mode ==0){//초반 구동 상대방 찾기(카메라 정중앙 들어오면 mode =1)
                    ...
                    if(data_R[2]==1){
                        if(ang <= angML){
                            ...
                            else if(data_R[1] == 1 || data_R[1]==2 || data_R[1]==3){//원통 작음 or 보통 or 큼
                                if(here==3) green_in_red_right();
                                else if (here==2) green_in_red_mid();
                                else green_in_red_left();
                            }
                            else if(data_R[1]==4 || data_R[1]==5){
                                if(here==3) green_in_red_right();
                                else if (here==2) green_in_red_mid();
                                else green_in_red_left();
                            ...
                            else if(data_R[1] == 1 || data_R[1]==2 || data_R[1]==3){
                                if(here==1) green_in_red_left();
                                else if (here==3) green_in_red_right();
                                else green_in_red_mid();
                            }
                            else if(data_R[1]==4 || data_R[1]==5){
                                if(here==1) green_in_red_left();
                                else if (here==3) green_in_red_right();
                        ...
                        angle_check();//각도 체크
                        // else if(imu_ang_chek_tmr.read_ms() > imu_ang_chek_time){
                        //     mode = 19;
                        // }
                    }
                    else if(data_R[2]==0){
                        //here = 0;
                            ...
                            else if(data_R[1] == 1 || data_R[1]==2 || data_R[1]==3){ // 화면 원통 작음 or 보통 or 큼
                                green_out_red_move();

                            }
                            else if(data_R[1]==4 || data_R[1]==5){ // 화면 원통 매우 큼 or 매우 매우 큼 or 매우 매우 매우 큼
                                    if(ang <= angLL && psdm_val <= 10){ // 앞 PSD 10cm 이하 + 각도 매우 왼쪽 : 매우 빠른 후진
                                        back_tmr_move<double>(&back_tmr, &back_escape_time, &psdm_val, "<", 20.0, &now_data, 70, -0.99, -0.6);

                                    }
                                    else if(ang >= angRR && psdm_val <= 10){ // 앞 PSD 10cm 이하 + 각도 매우 오른쪽 : 매우 빠른 후진
                                        back_tmr_move<double>(&back_tmr, &back_escape_time, &psdm_val, "<", 20.0, &now_data, 70, -0.6, -0.99);
...
//-----------------------------------------------------------다른 코드 ---------------------------------------------//
                    ...
                    //pc.printf("All black\n");
                        ...
                        //전방 ir이 빨간 색에 들어가고 상대가 멀리 있을 때 (공격시가 아닌 트래킹하는 상황)
                            ...
                            //전방 ir이 파란 색에 들어갔을 때 (공격시)
                                (ir_plusval[0]==true && now_data<100 && (data_R[1]==4 || data_R[1]==5)))
                            //전방 ir이 빨간 색에 들어가고 상대가 가까이 있을 때 (공격시)
                        {
                            if(ang>angML && ang<angMR && (data_R[0] != 999)){//공격시 색영역에 상대가 전방에 있으면 대기
                                ...
                                if(ang<90 && (data_R[1]==4 || data_R[1]==5)){//적이 좌측으로 탈출했을 때 좌회전해서 트래킹
                                    escape(&be_tmr,&escape_time2,-0.7,-0.7);
                                    escape(&be_tmr,&escape_time,-0.7,0.8);//while 대신 좌회전을 위한 mode 변경
                                }
                                else if(ang>=90 && (data_R[1]==4 || data_R[1]==5)){//적이 우측으로 탈출했을 때 우회전해서 트래킹
                                    escape(&be_tmr,&escape_time2,-0.7,-0.7);
                ...
                //servo_move(rcServo);
                    ...
                    mode = 1;
                     ...
                     angle_check();
                     servo_move(rcServo);
                //if(range_mode == 2 || range_mode ==3 || range_mode ==4){
                        // if(ir_plusval[0]==true){
                        //     speedL = -0.5;
                        //     speedR = 0.5;
                        // }
                            if(ir_val[5]<black && preread =='L'){
                                speedL = 0.8; speedR = 0.35;
                            }
                            else if(ir_val[5]<black && preread =='R'){
                                mode=1;
                                DC_follow();
                            }
                        // if((ir_val[1]<black && ir_val[0]>black && ir_val[3]<black) && ir_val[7]<black && preread =='R'){
                        //     speedL = 0.8; speedR = 0.35;
                        // }
                ...
                //if(range_mode == 2 || range_mode ==3 || range_mode ==4){
                        ...
                        //     speedR = -0.5;
                        // }
                        if(ir_val[6]<black && preread =='L'){
                            speedL = 0.35; speedR = 0.8;
                        }
                        else if(ir_val[6]<black && preread =='R'){
                            mode=1;
                            DC_follow();
                        }
                        // if((ir_plusval[6] == true && ir_val[3] > black && ir_val[2]<black) && preread =='L'){
                        //     speedL = 0.35; speedR = 0.8;
                        // }
                ...
                //*/
                //pc.printf("imu_ok\n");
                if(ir_val[7]>black && ir_val[8]>black && (data_R[1]==4 || data_R[1]==5)){
                    speedL = 0.99; speedR = 0.99;
                }
...
//---------------------------------------------------------------------//
            else if(All_mode >=3 && All_mode <5 ){//광운대용 전략 135도 돈 뒤 공격
                if(now_data>=40 && degree_mode ==0){
                    speedL = 0.5;speedR = -0.5;
                }
                else if(now_data < 40 ){
                    degree_135_tmr.start();
                    degree_mode += 1; 
                }
                if(degree_135_tmr.read_ms()<degree_135_time && degree_mode >=1){
                        speedL = 0.8; speedR = -0.8;
                }
                else if(degree_135_tmr.read_ms() >=degree_135_time && degree_135_tmr.read_ms() <= degree_135_time + 1100 && degree_mode >=1){
                        speedL = 0.90; speedR = 0.99;
                    }
                else if(degree_135_tmr.read_ms() > degree_135_time + 1100 && degree_mode >=1){
            ...
            else if(All_mode >=5){//광운대용 전략 45도 돈 뒤 공격
                if(now_data>=62 && degree_mode ==0){
                    speedL = 0.3;speedR = -0.3;
                }
                else if(now_data < 62 ){
                    degree_135_tmr.start();
                ...
                if(degree_135_tmr.read_ms()<1100 && degree_mode >=1){
                        speedL = 0.90; speedR = 0.99;
                }
                else if(degree_135_tmr.read_ms() >= 1100 && degree_mode >=1){
                        speedL = 0.0; speedR=0.0;
                        tmr_reset(&degree_135_tmr);
                        All_mode = 0;
                        mode =1;
                }
            }
            ...
            //com_check_tmr.start();
            if(com_check_tmr.read_ms() > com_chek_time){
