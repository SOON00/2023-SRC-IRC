#include "C:\Users\kts\Mbed Programs\mbed-os-empty-5\header.h"
#include "C:\Users\kts\Mbed Programs\mbed-os-empty-5\mpu6050.h"
#include <time.h>
#include <cstdint>
...
GP2A psdfr(PC_4,0.1, 0.8,0.23625 ,-0.297);
//GP2A psdf(PA_1,7,80,22.5,0.1606);//실험할 때 쓴 psd값
GP2A psdb(PA_7,20,150,60,0);
GP2A psdfm(PA_6,0.1, 0.8,0.23625 ,-0.297);
...
uint16_t black = 10000;//ir 검정색 바닥을 봤을 때
uint16_t imu_angle_black = 10000;
double speedL = 0;
double speedR = 0;
int mode =0;
...
int escape_time = 1000;
int com_chek_time = 1500;
int back_escape_time = 800;
double little_time = 1000;
double imu_ang_chek_time = 1500;
int blue_escape_time_ex = 3000;
int escape_blue_go_time = 2000;
int fight_back_escape_time = 630; 
int fight_back_break_check_time = 320; 
int waiting_break_time = 5000;
double escape_time2 = 500;
double degree_135_time = 25;
...
void servo_move(PwmOut &rc){
    ...
    if(gotPacket1){
        ...
        PID_control = map<float>(PID_control, 0. , 3500. , 0. ,14. );
...
void angle_check(){//ang값이 x도 이상일때 모드를 19로 바꿔주는 코드 추가
    if(abs(rollAngle)>5 || abs(pitchAngle) > 5){// ||abs(rollAngle)
        imu_ang_chek_tmr.start();
    }
    else if(abs(rollAngle) < 1.5 && abs(pitchAngle) < 1.5){
...
void DC_follow(){
    if(data_R[0]==999){//트래킹 중 상대가 보이지 않을 때
        ...
        if(psdm_val<20 || (ang>=40 && ang<=140)){
            speedL = 0.99;speedR = 0.99;
        }
        else{
            float delang = map<float>(abs(ang-90), 0. , 80. ,0.0 ,0.5 );
            if(ang>130){
                A = 1;
                B = -1;
                D=0;
            ...
            speedL = 0.49+ A*delang+D;
            speedR = 0.49+ B*delang+D;
        }
        //}
...
void green_in_red_left(){
    if(ir_plusval[7]==true){ // 모든 바퀴
        if(now_data >= 95){ // 뒤 PSD 70cm 이상 : 빠른 우회 후진 (ir 왼쪽 앞 바퀴, 오른쪽 앞 바퀴 검은색 될때까지, 시간 지나면 자동으로 빠져나옴)
        ...
        else if(now_data < 95){ // 뒤 PSD 70cm 이하 : 색 영역 탈출 모드
            blue_all_tmr_move();
        }
        ////////////////////////////////////////////////////////////////////////////////////////////////////
    }
    else if(ir_plusval[2] == true && ir_plusval[3] == true && ir_plusval[4] == false && ir_plusval[5] == false){ // 왼쪽 앞 바퀴 + 오른쪽 앞 바퀴 : 우회 후진 (ir 왼쪽 앞 바퀴, 오른쪽 앞 바퀴가 검은색일 때까지, 시간 지나면 자동으로 빠져나옴)
        back_tmr_move<bool>(&back_tmr, &back_escape_time, &ir_plusval[0], "=", true, &now_data, 70, -0.45, -0.30);
        //turn_90_tmr_move(&back_tmr, &escape_time, &ir_val[3], black, &data_R[1], -0.60, 0.60);
    }
    else if(ir_plusval[0]==true && ir_plusval[2] == false && ir_plusval[3] == false && ir_plusval[4] == false && ir_plusval[5] == false){ // ir 왼쪽 앞 + ir 오른쪽 앞 : 제자리 좌회전 (ir 가운데 앞 바퀴가 검은색일 때까지, 시간 지나면 자동으로 빠져나옴)

        turn_90_tmr_move(&back_tmr, &escape_time, &ir_val[3], black, &data_R[1], -0.60, 0.60);
        here = 1;//green_in_red_left 함수로 들어가서 원 타게
    }
    else if(ir_plusval[2]==true &&ir_plusval[3] ==false&& ir_plusval[4] ==false&& ir_plusval[5] ==false){ // 왼쪽 앞 바퀴 ???
        if(ir_plusval[0] == false){ // ir 왼쪽 앞 + ir 오른쪽 앞 X : 매우 오른쪽 전진
            speedL = 0.60; speedR = 0.10;//원타기
        ...
        (ir_plusval[2] == false && ir_plusval[3] == false && ir_plusval[5] == false && ir_plusval[4] == true) || // 왼쪽 뒷 바퀴 : 자유롭게 공격
        (ir_plusval[2] == false && ir_plusval[3] == false && ir_plusval[5] == true && ir_plusval[4] == false) || // 오른쪽 뒷 바퀴 : 자유롭게 공격
        (ir_plusval[2] == false && ir_plusval[3] == false && ir_plusval[5] == true && ir_plusval[4] == true) // 왼쪽 뒷 바퀴 + 오른쪽 뒷 바퀴 : 자유롭게 공격
    ){
        if(ang <= angLL){
...
void green_in_red_mid(){
    if(ir_plusval[7]==true){ // 모든 바퀴
       ...
       // if(ang >= 90){ // 서보 조금이라도 오른쪽 : 제자리 우회전 (ir 가운데 앞 바퀴가 검은색일 때까지, 시간 지나면 자동으로 빠져나옴)
            turn_90_tmr_move(&back_tmr, &escape_time, &ir_val[2], black, &data_R[1], 0.60, -0.60);
            here=1;//green_in_red_mid 함수로 들어가서 원 타게
        //}
        // else if(ang < 90){ // 서보 조금이라도 왼쪽 : 제자리 좌회전 (ir 가운데 앞 바퀴가 검은색일 때까지, 시간 지나면 자동으로 빠져나옴)
        //     turn_90_tmr_move(&back_tmr, &escape_time, &ir_val[3], black, &data_R[1], -0.60, 0.60);
        //     here=3;//green_in_red_mid 함수로 들어가서 원 타게
        // }
    }
...
void green_in_red_right(){
    if(ir_plusval[2] == true && ir_plusval[3] == true && ir_plusval[5] == true && ir_plusval[4] == true){ // 모든 바퀴
        ...
        turn_90_tmr_move(&back_tmr, &escape_time, &ir_val[2], black, &data_R[1], 0.60, -0.60);
        here=3;//green_in_red_right 함수로 들어가서 원 타게
    }
    else if(ir_plusval[2] == false && ir_plusval[3] == true && ir_plusval[5] == false && ir_plusval[4] == false){ // 오른쪽 앞 바퀴
        if(ir_plusval[0] == false){ // ir 왼쪽 앞 + ir 오른쪽 앞 X : 매우 왼쪽 전진
            speedL = 0.10; speedR = 0.60;//원타기
    ...
    if(blue_escape_tmr.read_ms() < blue_escape_time_ex){
        if( // psdf 2개 이상 거리 가까움 : 매우 빠른 탈출
            ...
            while(ir_val[0] < black && ir_val[1] < black){ // ir 왼쪽 앞 검정색 + ir 오른쪽 앞 검정색 : break
                if(ang < angMM){
                    speedL = map<float>(ang, 0.0, angMM, 0.80, 0.99);
                    speedR = 0.99;
                }
                else if(ang >= angMM){
                    speedL = 0.99;
                    speedR = map<float>(ang, 180.0, angMM, 0.80, 0.99);
                }
        ...
        if(angLL < ang){ // 서보 보통 왼쪽
            if(ir_plusval[2] == true && ir_plusval[3] == true && ir_plusval[5] == true && ir_plusval[4] == true){ // 모든 바퀴
                if(now_data >= 95){ // 뒤 PSD 70cm 이상 : 우회 후진 (ir 왼쪽 앞 바퀴, 오른쪽 앞 바퀴 검은색 될때까지, 시간 지나면 자동으로 빠져나옴)
                    ...
                    if((data_R[1]==4||data_R[1]==5)){
                        fight_back_tmr_move(&back_tmr, &fight_back_escape_time, &fight_back_break_check_time, 0.99, 0.99);
                    }
                }
            ...
            if((data_R[1]==4||data_R[1]==5)){ // 화면 원통 매우 매우 or 매우 매우 매우 큼
                speedL = -0.99; speedR = 0.99;
            }
...
void imu_tmr_move(){
    ...
    if(ang <= angML){ // 서보 왼쪽
        if(angLL < ang){ // 서보 보통 왼쪽
            // 항상 : 자유롭게 공격
            if(
                (ir_val[7] > imu_angle_black && ir_val[3] > black && ir_val[4] > black && ir_val[8] > imu_angle_black) || // ir 전부 검정 : 자유로운 공격
                (ir_val[7] > imu_angle_black && ir_val[3] > black && ir_val[4] < black && ir_val[8] > imu_angle_black) || // ir 왼쪽 뒤 색 : 자유로운 공격
                (ir_val[7] > imu_angle_black && ir_val[3] < black && ir_val[4] > black && ir_val[8] > imu_angle_black) || // ir 오른쪽 뒤 색 : 자유로운 공격
                (ir_val[7] > imu_angle_black && ir_val[3] < black && ir_val[4] < black && ir_val[8] > imu_angle_black) ||
                (ir_val[7]> imu_angle_black && ir_val[8] >imu_angle_black ) // ir 왼쪽 뒤 색 + 오른쪽 뒤 색 : 자유로운 공격
            ){
                green_out_red_move();
            }
            // 상대 좌측 : 자유롭게 공격
            else if(
                (ir_val[7] < imu_angle_black && ir_val[3] > black && ir_val[4] > black && ir_val[8] > imu_angle_black) || // ir 오른쪽 가운데 색 : 자유로운 공격
                (ir_val[7] < imu_angle_black && ir_val[3] < black && ir_val[4] > black && ir_val[8] > imu_angle_black) || // ir 오른쪽 가운데 색 + 오른쪽 뒤 색 : 자유로운 공격
                (ir_val[7] < imu_angle_black && ir_val[3] < black && ir_val[4] < black && ir_val[8] > imu_angle_black) ||
                (ir_val[7] < imu_angle_black && ir_val[8] >imu_angle_black )// ir 오른쪽 가운데 색 + 오른쪽 뒤 색 + 왼쪽 뒤 색 : 자유로운 공격
            ){
                green_out_red_move();
            }
            // 항상 : 빠른 오른쪽 후진
            else if(
                (ir_val[7] < imu_angle_black && ir_val[3] > black && ir_val[4] > black && ir_val[8] < imu_angle_black) || // ir 오른쪽 가운데 + 왼쪽 가운데 색 : 빠른 오른쪽 후진
                (ir_val[7] < imu_angle_black && ir_val[3] > black && ir_val[4] < black && ir_val[8] < imu_angle_black) || // ir 오른쪽 가운데 + 왼쪽 뒤 색 + 왼쪽 가운데 색 : 빠른 오른쪽 후진
                (ir_val[7] < imu_angle_black && ir_val[3] < black && ir_val[4] > black && ir_val[8] < imu_angle_black) || // ir 오른쪽 가운데 + 오른쪽 뒤 색 + 왼쪽 가운데 색 : 빠른 오른쪽 후진
                (ir_val[7] < imu_angle_black && ir_val[3] < black && ir_val[4] < black && ir_val[8] < imu_angle_black)||
                (ir_val[7]< imu_angle_black && ir_val[8] < imu_angle_black ) // ir 오른쪽 가운데 + 오른쪽 뒤 색 + 왼쪽 뒤 색 + 왼쪽 가운데 색 : 빠른 오른쪽 후진
            ){
                back_tmr_move<float>(&back_tmr, &back_escape_time, &rollAngle, "<", -1.5, &now_data, 70, -map<float>(ang, angML, angLL, 0.60, 0.85) * 1.1647, -0.50 * 1.1647);
            }
            // 상대 좌측 : 빠른 오른쪽 후진
            else if(
                (ir_val[7] > imu_angle_black && ir_val[3] > black && ir_val[4] > black && ir_val[8] < imu_angle_black) || // ir 왼쪽 가운데 색 : 빠른 오른쪽 후진
                (ir_val[7] > imu_angle_black && ir_val[3] > black && ir_val[4] < black && ir_val[8] < imu_angle_black) || // ir 왼쪽 뒤 색 + 왼쪽 가운데 색 : 빠른 오른쪽 후진
                (ir_val[7] > imu_angle_black && ir_val[3] < black && ir_val[4] < black && ir_val[8] < imu_angle_black) ||
                (ir_val[7] < imu_angle_black && ir_val[8] < imu_angle_black )// ir 오른쪽 뒤 색 + 왼쪽 뒤 색 + 왼쪽 가운데 색 : 빠른 오른쪽 후진
            ){
                back_tmr_move<float>(&back_tmr, &back_escape_time, &rollAngle, "<", -1.5, &now_data, 70, -map<float>(ang, angML, angLL, 0.60, 0.85) * 1.1647, -0.50 * 1.1647);
            }
            // 존재하지 않는 경우 : 자유롭게 공격
            ...
            // 항상 : 빠른 오른쪽 후진
                ...
                back_tmr_move<float>(&back_tmr, &back_escape_time, &rollAngle, "<", -1.5, &now_data, 70, -map<float>(ang, angLL, 0.0, 0.85, 0.95) * 1.042, -0.50 * 1.0525);
            }
            // 상대 좌측 : 빠른 오른쪽 후진
            else if(
                (ir_val[7] > imu_angle_black && ir_val[3] > black && ir_val[4] > black && ir_val[8] < imu_angle_black) || // ir 왼쪽 가운데 색 : 빠른 오른쪽 후진
